...
function DashboardLayout({ user, handleLogout }) {
  ...
  const handleSignUp = async (e) => {
    ...
    if (error) alert(error.message);
    else alert('Signup check successful! Please check email or login directly.');
    setLoading(false);
  };
    ...
    <div className="flex h-screen bg-[#FFFDF9] text-[#37352f] overflow-hidden">
      ...
      <aside className="w-64 bg-[#F7F4EB] border-r-2 border-[#EADFC9] flex flex-col justify-between p-4 select-none">
        <div>
          ...
          <div className="mt-8 space-y-2">
            ...
            <div className="flex items-center space-x-3 px-4 py-2.5 text-gray-500 rounded-xl text-sm font-bold">
      ...
      <main className="flex-1 flex flex-col overflow-y-auto">
        ...
        <div className="max-w-4xl w-full mx-auto px-12 pt-10 pb-24">
          ...
          {/* AUTH GATE GATEWAY IN THE MIDDLE */}
          {!user ? (
...
// SECURE BOOTSTRAP FOR APP PATHS
export default function App() {
  ...
  return (
    <Router>
      <Routes>
        {/* / path kholte hi beautiful restored original look layout load hoga */}
        <Route path="/" element={<DashboardLayout user={user} handleLogout={handleLogout} />} />
        {/* Kisi mismatch hone par bypass redirect handler */}
