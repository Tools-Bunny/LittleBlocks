    ...
    <div className="flex h-screen bg-[#FFFDF9] text-[#37352f] overflow-hidden">
      ...
      <aside className="w-64 bg-[#F7F4EB] border-r-2 border-[#EADFC9] flex flex-col justify-between p-4 select-none">
        ...
        <div className="p-3 bg-white border-2 border-[#EADFC9] rounded-2xl shadow-sm">
          <label className="text-[10px] uppercase font-black tracking-widest text-amber-600 block mb-1.5">👩‍👦 Mother's Tongue</label>
          <select 
            value={selectedLang} 
      ...
      <main className="flex-1 flex flex-col overflow-y-auto">
        ...
        <div className="max-w-4xl w-full mx-auto px-12 pt-10 pb-24">
          ...
          {/* Conditional Rendering: If not logged in, show Auth Gate */}
          {!user ? (
            <div className="max-w-sm mx-auto bg-white border-2 border-[#EADFC9] p-8 rounded-3xl shadow-sm mt-12">
            ...
            <>
              <div className="w-full bg-gradient-to-r from-[#FF6B6B] via-[#FFD166] to-[#4ECDC4] h-32 rounded-3xl p-6 relative overflow-hidden shadow-md mb-8 flex items-center">
                <div className="relative z-10 text-white">
                  <span className="text-sm uppercase font-black tracking-widest bg-black/20 px-2 py-0.5 rounded-md">Cloud Synced Workspace</span>
                  <h2 className="text-2xl font-black mt-1">Hello Cloud Mom! Your data is safe.</h2>
                </div>
                <span className="text-6xl absolute right-6">🦖</span>
              </div>

              <div className="flex items-center space-x-4 mb-4">
                <span className="text-5xl bg-white p-2 rounded-2xl border-2 border-gray-100 shadow-sm">🎯</span>
                <h1 className="text-3xl font-black text-gray-800 tracking-tight">{content.title}</h1>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-10">
                ...
                <div className="bg-white border-2 border-[#EADFC9] p-6 rounded-3xl shadow-sm space-y-4">
                  <div className="space-y-3">
                    {tasks.map((task) => (
                      ...
                      >
                        <div className="flex items-center space-x-3">
                          <div className={`w-6 h-6 rounded-xl flex items-center justify-center border-2 ${task.done ? 'bg-emerald-500 border-emerald-600' : 'bg-white'}`}>
                            {task.done && <span className="text-white text-xs font-black">✓</span>}
                          </div>
                          <span className={`text-sm font-bold ${task.done ? 'line-through' : ''}`}>{content[task.textKey]}</span>
                        </div>
                ...
                <div className="space-y-6">
                  <div className="bg-white border-2 border-[#EADFC9] p-6 rounded-3xl shadow-sm space-y-4">
                    <div className="border-2 border-red-200 rounded-2xl p-4 bg-rose-50/50 flex items-center justify-between">
                      <div className="flex items-center space-x-4">
                        <div className="w-12 h-12 bg-[#FF6B6B] text-white flex items-center justify-center rounded-2xl text-xl font-bold">▶️</div>
                        <div>
                          <h4 className="text-base font-extrabold text-gray-800">{content.story}</h4>
                          <p className="text-xs font-bold text-[#FF6B6B]">Audio Playback • Vernacular</p>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-amber-50 border-2 border-dashed border-amber-300 p-5 rounded-3xl">
                    <h4 className="text-xs font-black text-amber-700">❤️ Mamma's Shabaash Board</h4>
